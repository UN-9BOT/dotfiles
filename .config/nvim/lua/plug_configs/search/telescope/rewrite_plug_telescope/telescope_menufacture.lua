-- rewrite https://github.com/molecule-man/telescope-menufacture
-- NOTE: не смог пропатчить grep_string, не получает prompt_value
-- добавлен toggle_flags_from_string
-- доmбавлен menu_actions.references
...
M.toggle_flag = function(flag_key, flag_value)
  ...
  return function(opts, callback)
    ...
    -- opts.prompt_title = opts.prompt_title or "nil" .. " " .. flag_value
    callback(opts)
  end
end
        ...
        for mode, key in pairs(mappings) do
          if actions_indexed_by_name[action_name] then
            map(
              mode,
              key,
              action_mapper(actions_indexed_by_name[action_name]),
              { desc = "menufacture|" .. action_name }
            )
          end
        end
      end
...
--- @param flag_string string например "--hidden --no-ignore"
M.toggle_flags_from_string = function(flag_string)
  local key = "additional_args" -- Всегда работаем с additional_args

  return function(opts, callback)
    -- Разбиваем строку флагов на отдельные флаги
    local flags = vim.split(flag_string, "%s+")

    -- Создаём таблицу с уникальными флагами
    local existing_flags = opts[key] or {}
    if type(existing_flags) == "function" then
      existing_flags = existing_flags(opts)
    end

    -- Собираем новый список флагов, удаляя старые
    local new_flags = {}
    local flag_set = {}
    for _, v in pairs(existing_flags) do
      flag_set[v] = true
    end
    for _, v in ipairs(flags) do
      if flag_set[v] then
        flag_set[v] = nil -- Если флаг уже был, убираем его (переключаем)
      else
        table.insert(new_flags, v) -- Добавляем новые флаги
      end
    end

    -- Записываем обновлённые флаги в opts
    opts[key] = new_flags
    callback(opts)
  end
end
...
M.menu_actions = {
  ...
  toggle_flag_extra_hidden = {
    action_name = "toggle_extra_hidden",
    action = M.toggle_flags_from_string("--hidden --no-ignore"),
    text = "toggle extra hidden (hidden + no-ignore)",
  },
...
set_menu(M.live_grep_menu, M.menu_actions.toggle_flag_extra_hidden)
...
set_menu(M.grep_string_menu, M.menu_actions.toggle_flag_extra_hidden)
...
M.references_menu = {}
set_menu(M.references_menu, M.menu_actions.toggle_flag_extra_hidden)
set_menu(M.references_menu, M.menu_actions.toggle_flag_no_ignore)
set_menu(M.references_menu, M.menu_actions.toggle_flag_hidden)
set_menu(M.references_menu, M.menu_actions.search_in_directory)
...
M.references = M.add_menu_with_default_mapping(builtin.lsp_references, M.references_menu)
